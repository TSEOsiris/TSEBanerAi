...
namespace OverlayTest.LLM
{
    ...
    public class LLMRequestStatus
    {
        ...
        public List<DebugLogEntry> DebugLog { get; set; } = new();
        ...
        public string GetNormalModeText()
        {
            return State switch
            {
                LLMRequestState.Thinking => $"{NpcName} думает...",
                LLMRequestState.FetchingContext => $"{NpcName} вспоминает...",
                LLMRequestState.Generating => $"{NpcName} формирует ответ...",
                _ => ""
            };
        }
        ...
        public string GetDebugModeText()
        {
            return State switch
            {
                LLMRequestState.Thinking => "Отправка запроса к LLM...",
                LLMRequestState.FetchingContext => $"Запрос контекста: {string.Join(", ", ContextSources)}",
                LLMRequestState.Generating => $"Генерация ответа ({TokensGenerated}/{TokensTotal} tokens)",
                LLMRequestState.Complete => "Ответ получен",
                _ => "Ожидание..."
            };
        }
        ...
        /// </summary>
        public bool IsActive => State != LLMRequestState.Idle && State != LLMRequestState.Complete;
